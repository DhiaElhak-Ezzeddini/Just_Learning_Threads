...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  // Masks
  __cs149_mask maskAll, maskIsZero, maskIsNotZero, maskIsGreaterThanClamp;

  maskAll = _cs149_init_ones(); // masking all 
  int i;
  for (i = 0; i + VECTOR_WIDTH <= N; i += VECTOR_WIDTH){
    result = _cs149_vset_float(1.f); // result = 1.f
    _cs149_vload_float(x, values+i , maskAll);  // x = values[i]
    _cs149_vload_int(y, exponents+i , maskAll); // y = exponents[i]
    __cs149_mask mult;

    _cs149_vgt_int(mult, y , zero, maskAll);

    while(_cs149_cntbits(mult)){
      _cs149_vmult_float(result,result,x,mult); // result = result*x
      _cs149_vsub_int(y,y,one,mult); // y = y - 1
      _cs149_vgt_int(mult, y , zero, mult);
    }
    ...
    //_cs149_vmult_float(result,x ,x ,maskIsNotZero); //   result = x*x;
    //_cs149_vsub_int(y,y, _cs149_vset_int(1), maskIsNotZero); //   y = y - 1;

    _cs149_vgt_float(maskIsGreaterThanClamp, result, clamp, maskAll); // if (result > 9.999999f) {
    _cs149_vset_float(result, 9.999999f, maskIsGreaterThanClamp); //   result = 9.999999f;
